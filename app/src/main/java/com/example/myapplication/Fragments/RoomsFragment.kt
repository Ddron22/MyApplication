...
class AudienceFragment : Fragment() {
    ...
    var database = FirebaseDatabase.getInstance().getReference("Rooms")
        ...
        var getdata = object : ValueEventListener {
            override fun onDataChange(p0: DataSnapshot) {
                for(i in p0.children){
                    ...
                    var number_of_seats = i.child("number_of_seats").getValue().toString().toInt()
                    (activity as MainActivity?)?.newDataSecond(id as Int,
                        number as String,
                        name as String,
                        number_of_seats as Int
                    )
                }
            }
